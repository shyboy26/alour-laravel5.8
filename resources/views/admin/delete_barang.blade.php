				...
				<div class="modal-dialog">
					...
					<div class="modal-content">
						<div class="modal-header">
							<h4 class="modal-tittle">Konfirmasi Hapus Barang</h4>
							<button type="button" class="close"><a href="data_barang.php" style="color: black">&times;</a></button>
						</div>
						<div class="modal-body">
							<form action="/admin/data_barang/delete/{{ $id }}" method="post">
                                {{ csrf_field() }}
                                {{ method_field('PUT') }}
                                <div class="form-group">
									<label>Yakin ingin menghapus barang ini ?</label>
									
								</div>
								
								<input name="btn_batal" type="submit" class="btn btn-success btn-sm" value="Batal" style="float: right;width: 80px" />
								<input name="btn_delete" type="submit" class="btn btn-danger btn-sm" value="Hapus" style="float: right;margin-right: 10px;width: 80px" />
							</form>
						</div>
					</div>
				</div>
